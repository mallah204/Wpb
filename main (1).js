...
const siminfoCommand = require('./commands/siminfo');
...
const zuckCommand = require('./commands/zuck');
...
global.channelLink = "https://whatsapp.com/channel/0029Vb7Svri7oQhZNL7e5u2b";
global.ytch = "Its Raza";
...
const alexaCommand = require('./commands/alexa');
const editCommand = require('./commands/edit');

const uptimeCommand = require('./commands/uptime');

const datajsonHandler = require('./commands/datajson_handler');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        const senderId = message.key.participantAlt || message.key.remoteJid;
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                ...
                await sock.sendMessage(chatId, {
                    text: `🔗 *Support*\n\nhttps://whatsapp.com/channel/0029Vb7Svri7oQhZNL7e5u2b?mode=wwt`
                }, { quoted: message });
                return;
            }
        }
        ...
        const ownerCommands = ['mode', 'autostatus', 'antidelete', 'cleartmp', 'setpp', 'clearsession', 'areact', 'autoreact', 'autotyping', 'autoread', 'pmblocker', 'zuck', 'xvideo'].map(c => prefix + c);
        ...
        switch (true) {
            ...
            case userMessage.startsWith(prefix + 'mute'):
                {
                    ...
                    if (muteArg !== undefined && (isNaN(muteDuration) || muteDuration <= 0)) {
                        await sock.sendMessage(chatId, { text: 'Please provide a valid number of minutes or use ' + prefix + 'mute with no number to mute immediately.', ...channelInfo }, { quoted: message });
                    } else {
            ...
            case userMessage.startsWith(prefix + 'unban'):
                ...
                break;
            case userMessage.startsWith(prefix + 'alexa'):
                await alexaCommand(sock, chatId, message);
                break;
            case userMessage.startsWith(prefix + 'edit'):
            ...
            case userMessage.startsWith(prefix + 'tts'):
                const text = userMessage.slice(prefix.length + 3).trim();
            ...
            case userMessage.startsWith(prefix + 'delete') || userMessage.startsWith(prefix + 'del'):
                ...
                break;
            case userMessage.startsWith(prefix + 'reload'):
                const reloadArg = userMessage.split(' ')[1]?.trim();
                await reloadCommand(sock, chatId, message, reloadArg);
                break;
            ...
            case userMessage.startsWith(prefix + 'mode'):
                // ... [Existing code for mode]
                break;
            case userMessage.startsWith(prefix + 'pinterest'):
                await pinterestCommand(sock, chatId, message);
                break;
            case userMessage.startsWith(prefix + 'tempmail'):
                await tempmailCommand(sock, chatId, message);
                break;
            case userMessage.startsWith(prefix + 'siminfo'):
                await siminfoCommand(sock, chatId, message);
                break;
            case userMessage.startsWith(prefix + 'ai'):
                await aiCommand(sock, chatId, message);
                break;
            case userMessage.startsWith(prefix + 'zuck'):
                if (!message.key.fromMe && !senderIsOwnerOrSudo) {
                    await sock.sendMessage(chatId, { text: '❌ This command is only available for the owner or sudo!' }, { quoted: message });
                    return;
                }
                await zuckCommand(sock, chatId, message);
                break;
            ...
            case userMessage.startsWith('.character'):
                ...
                break;
            case userMessage.startsWith('.suno'):
                await sunoCommand(sock, chatId, message);
                break;
            ...
            case userMessage.startsWith('.heart'):
                ...
                break;
            case userMessage.startsWith('.zuck'):
                await zuckCommand(sock, chatId, message);
                break;
            ...
            case userMessage.startsWith('.anime'):
                {
                    const cmd = 'anime';
                    await datajsonHandler(sock, chatId, message, cmd);
                    commandExecuted = true;
                }
                break;
            ...
            case userMessage.startsWith('.'): {
                const cmd = userMessage.slice(1).split(' ')[0];
                const availableFiles = ['anime', 'ausand', 'chitanda', 'dú', 'gaivip', 'gura', 'icon', 'ig', 'kana', 'kurumi', 'loli', 'mirai', 'mông', 'mui', 'neko', 'phongcanh', 'poem', 'rem', 'sagiri', 'siesta', 'thinh', 'umaru', 'vdanime', 'vdcosplay', 'vdgai'];
                if (availableFiles.includes(cmd)) {
                    await datajsonHandler(sock, chatId, message, cmd);
                    commandExecuted = true;
                }
                break;
